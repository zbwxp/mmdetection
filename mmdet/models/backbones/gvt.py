...
class OldAttention(Attention):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        super(OldAttention, self).__init__(dim, num_heads, qkv_bias, qk_scale, attn_drop, proj_drop, sr_ratio)
        del self.q, self.kv
        self.qkv = nn.Linear(dim, dim*3, bias=qkv_bias)
    ...
    def forward(self, x, H, W):
        ...
        q = self.qkv(x)[:, :, 0:C].reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
            x_ = self.norm(x_)
            kv = self.qkv(x_)[:, :, C:].reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        else:
            kv = self.qkv(x)[:, :, C:].reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
...
class OldGroupBlock(GroupBlock):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, ws=1):
        super(OldGroupBlock, self).__init__(dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop,
                                         drop_path, act_layer, norm_layer)
        del self.attn
        if ws == 1:
            self.attn = OldAttention(dim, num_heads, qkv_bias, qk_scale, attn_drop, drop, sr_ratio)
        else:
            self.attn = GroupAttention(dim, num_heads, qkv_bias, qk_scale, attn_drop, drop, ws)
...
class PosCNN(nn.Module):
    def __init__(self, in_chans, embed_dim=768, s=1):
        ...
        # self.proj = nn.Conv2d(in_chans, embed_dim, 3, 1, padding=1, bias=True, groups=embed_dim)
...
class GVT(PCPVT):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
                 num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[4, 4, 4], sr_ratios=[4, 2, 1], block_cls=GroupBlock, wss=[7, 7, 7], F4=False, extra_norm=False):
        super(GVT, self).__init__(img_size, patch_size, in_chans, num_classes, embed_dims, num_heads,
                                    mlp_ratios, qkv_bias, qk_scale, drop_rate, attn_drop_rate, drop_path_rate,
                                    norm_layer, depths, sr_ratios, block_cls, F4)
        del self.blocks
        self.ws = wss
        ...
        for k in range(len(depths)):
            _block = nn.ModuleList([block_cls(
                dim=embed_dims[k], num_heads=num_heads[k], mlp_ratio=mlp_ratios[k], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer, sr_ratio=sr_ratios[k],
                ws=1 if i % 3 == 0 and k != 0 else wss[k]) for i in range(depths[k])])
            self.blocks.append(_block)
            cur += depths[k]
        self.apply(self._init_weights)
...
@BACKBONES.register_module()
class gvt_small_norm(GVT):
    def __init__(self, **kwargs):
        super(gvt_small_norm, self).__init__(
            patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 10, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, extra_norm=True)


@BACKBONES.register_module()
class gvt_base(GVT):
    def __init__(self, **kwargs):
        super(gvt_base, self).__init__(
            patch_size=4, embed_dims=[64, 160, 320, 640], num_heads=[2, 5, 10, 20], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 18, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.2)


@BACKBONES.register_module()
class gvt_base_norm(GVT):
    def __init__(self, **kwargs):
        super(gvt_base_norm, self).__init__(
            patch_size=4, embed_dims=[64, 160, 320, 640], num_heads=[2, 5, 10, 20], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 18, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.2, extra_norm=True)


@BACKBONES.register_module()
class gvt_base_norm_g(GVT):
    def __init__(self, **kwargs):
        """
        global feature is taken
        """
        super(gvt_base_norm_g, self).__init__(
            patch_size=4, embed_dims=[64, 160, 320, 640], num_heads=[2, 5, 10, 20], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 18, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.2, extra_norm=True)
    ...
    def forward_features(self, x):
        ...
        for i in range(len(self.depths)):
            ...
            for j, blk in enumerate(self.blocks[i]):
                ...
                if j == 0:
                    x = self.pos_block[i](x, H, W)
                if i == 2 and j == len(self.blocks[i]) - 2:
                    outputs.append(x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous())
...
@BACKBONES.register_module()
class gvt_large_norm(GVT):
    def __init__(self, **kwargs):
        super(gvt_large_norm, self).__init__(
            patch_size=4, embed_dims=[96, 192, 384, 768], num_heads=[3, 6, 12, 24], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 18, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.25)


@BACKBONES.register_module()
class gvt_large_old(GVT):
    def __init__(self, **kwargs):
        super(gvt_large_old, self).__init__(
            patch_size=4, embed_dims=[96, 192, 384, 768], num_heads=[3, 6, 12, 24], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 18, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, block_cls=OldGroupBlock)


@BACKBONES.register_module()
class gvt_huge(GVT):
    def __init__(self, **kwargs):
        super(gvt_huge, self).__init__(
...
@BACKBONES.register_module()
class gvt_small_f4(GVT):
    def __init__(self, **kwargs):
        super(gvt_small_f4, self).__init__(
            patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 10, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, F4=True)
...
class alt_gvt_small(ALTGVT):
    def __init__(self, **kwargs):
        super(alt_gvt_small, self).__init__(
            patch_size=4, embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 10, 4], wss=[7, 7, 7, 7], sr_ratios=[8, 4, 2, 1],
            extra_norm=False, drop_path_rate=0.2,
        )
...
@BACKBONES.register_module()
class pcpvt_tiny(CPVTV2):
    def __init__(self, **kwargs):
        super(pcpvt_tiny, self).__init__(
